...
def alterar_nome():
    ...
    if user:
        ...
        session.commit()
        flask_session['username'] = novo_nome  # Atualiza a sessão com o novo nome
...
def procurar_abastecimento():
    ...
    if not placa or not mes or not ano:
        return jsonify([])  # Adicionada a verificação de campos vazios
...
def cadastrar_abastecimento():
    ...
    try:
        data_req = datetime.strptime(data_req, "%Y-%m-%d")  # Converter para objeto datetime
    except ValueError:
    ...
    # Lógica para obter o abastecimento anterior
    ...
    if abastecimento_anterior:
        ...
        # Verifica se o km_por_litro está abaixo da média esperada para o veículo
